...
def _restart_adb(path_info):
	...
	run_detached([path_info.adb, 'start-server'])
	time.sleep(5)
...
def run_detached(args):
	# Run a process entirely detached from this one
	if sys.platform.startswith('win'):
		os.system("cmd /c start "+" ".join(args))
	else:
		os.system("bash -c '"+" ".join(args)+" >/dev/null 2>&2 &' >/dev/null 2>&2 &")
